...
class MemgraphTPCH:
    # They really want from us to use neo4j driver to connect to Memgraph?
    # Well, okay...
    def __init__(self, config: dict[str, Any] | None = None) -> None:
        """
        Initialize the MemgraphTPCH instance with configuration details.
    ...
    async def adrop(self) -> None:
        ...
        async with self.driver.session() as session:
            indices = [
                ...
                "DROP INDEX ON :Customer(nationkey)",
                "DROP INDEX ON :Supplier(nationkey)",
                "DROP INDEX ON :Nation(regionkey)",
                "DROP INDEX ON :Order(custkey)",
                "DROP INDEX ON :LineItem(orderkey)",
                "DROP INDEX ON :LineItem(partkey)",
                "DROP INDEX ON :LineItem(suppkey)",
            ]
